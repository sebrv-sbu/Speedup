...
dist_data_frame <- read.csv("/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/lin105/fracscore/csvs/dist_data_frame.csv", header=TRUE)
orig_data_s <- read.csv("/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/lin105/fracscore/csvs/orig_data_s.csv", header=TRUE)
tot_int_data <- read.csv("/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/lin105/fracscore/csvs/tot_int_data.csv", header=TRUE)
data_univ <- read.csv("/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/lin105/fracscore/csvs/data_univ.csv", header=TRUE)
...
ggsave(YuefanPlot, filename="/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/lin105/fracscore/pdfs/speedup.svg", height=5, width=5*1.5)
ggsave(YuefanPlot, filename="/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/lin105/fracscore/pdfs/speedup.pdf", height=5, width=5*1.5)
Speedup105 <- YuefanPlot
...
NewPlot <- NewPlot + scale_x_reverse(name="E/E_min", 
                                     ...
                                     labels=lapply(seq(max_bin_E,min_bin_E, by = -2),
                                                   transform_bin_E))
ggsave(NewPlot, filename="/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/lin105/fracscore/pdfs/DensityAndEnergy.pdf")
DensityAndEnergy105 <- NewPlot
...
new_graph_data <- new_graph_data[which(((new_graph_data$P == 200 & new_graph_data$m_0 == 1000 & new_graph_data$m_1 == 1000) |(
                                  (( (new_graph_data$P == 100 & new_graph_data$m_0 == 1000 & new_graph_data$m_1 == 100)  )|
                                 ((new_graph_data$P == 20  & new_graph_data$m_0 == 50 & new_graph_data$m_1 == 100) |
                                 (new_graph_data$P == 20  & new_graph_data$m_0 == 150  & new_graph_data$scheme == "cdr")) |
                                 (((new_graph_data$P == 10  & new_graph_data$m_0 == 40)|
                                 (new_graph_data$P == 5   & new_graph_data$m_0 == 30))|
...
ggsave(BoxGraph, filename="/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/lin105/fracscore/pdfs/EnergyBoxplot.pdf")
BoxGraph105 <- BoxGraph
dist_data_frame <- read.csv("/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/kroA200/fracscore/csvs/dist_data_frame.csv", header=TRUE)
orig_data_s <- read.csv("/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/kroA200/fracscore/csvs/orig_data_s.csv", header=TRUE)
tot_int_data <- read.csv("/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/kroA200/fracscore/csvs/tot_int_data.csv", header=TRUE)
data_univ <- read.csv("/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/kroA200/fracscore/csvs/data_univ.csv", header=TRUE)
dist_data_frame$X <- NULL
orig_data_s$X <- NULL
tot_int_data$X <- NULL
K_E_max_yax <- 328
...
#The following is to pick the intervals we want to represent in the graph.#
graph_data <- tot_int_data[which(((tot_int_data$P == 200 & tot_int_data$m_0 == 1000 & tot_int_data$m_1 == 1000) |(
                                  (( (tot_int_data$P == 100 & tot_int_data$m_0 == 1000 & tot_int_data$m_1 == 50)  )|
...
graph_data$bin_E <- graph_data$bin_E #-0.4
graph_data$bin_I <- graph_data$bin_I #-0.5
YuefanPlot <- ggplot(data = graph_data, aes(y=bin_I, x=bin_E, color=interaction(P,scheme), shape=scheme))+geom_point(stroke=1)
YuefanPlot <- YuefanPlot + scale_shape_manual(values=c(1,5,0))
YuefanPlot <- YuefanPlot + scale_color_manual(values=c("green", "red","blue","black","brown","magenta","orange"))
YuefanPlot <- YuefanPlot + scale_y_reverse(name="log(I*P)",breaks=seq(max_bin_I, min_bin_I, by=-15),expand=c(0.05,0.05),labels= lapply(seq(max_bin_I, min_bin_I, by=-15), transform_bin_I))
YuefanPlot <- YuefanPlot + scale_x_reverse(name="E/E_min",breaks=seq(max_bin_E,min_bin_E, by=-2), expand=c(0.05,0.05), labels=lapply(seq(max_bin_E, min_bin_E, by=-2), transform_bin_E))
...
ggsave(YuefanPlot, filename="/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/kroA200/fracscore/pdfs/speedup.svg")
ggsave(YuefanPlot, filename="/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/kroA200/fracscore/pdfs/speedup.pdf")
Speedup200 <- YuefanPlot
#procs=c(5,10,20,100,200)
#schemes = c("cdr", "hsa")
#for (proc in procs){
#  for (scheme in schemes){
...
NewPlot <- NewPlot + scale_x_reverse(name="E/E_min", 
                                     ...
                                     labels=lapply(seq(max_bin_E,min_bin_E, by = -2),
                                                   transform_bin_E))
ggsave(NewPlot, filename="/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/kroA200/fracscore/pdfs/DensityAndEnergy.pdf")
DensityAndEnergy200 <- NewPlot
...
ggsave(BoxGraph, filename="/users/seb/Hierarchical-Simulated-Annealing/Speedup_Sebmethod/kroA200/fracscore/pdfs/EnergyBoxplot.pdf")

BoxGraph200 <- BoxGraph
Speedup <- ggarrange(Speedup105, Speedup200, ncol=2, nrow=1, common.legend=TRUE)
DensityAndEnergy <- ggarrange(DensityAndEnergy105, DensityAndEnergy200, ncol=2, nrow=1,
common.legend=TRUE)
ggsave(DensityAndEnergy, filename="pdfs/DensityAndEnergy.pdf", width=15, height=7)
ggsave(DensityAndEnergy, filename="pdfs/DensityAndEnergy.svg", width=15, height=7)
DensityAndEnergy105 <- DensityAndEnergy105+theme(legend.title=element_blank(), legend.position="none")
DensityAndEnergy200 <- DensityAndEnergy200+theme(legend.title=element_blank(), legend.position="none")
DensityAndEnergy <- ggarrange(DensityAndEnergy105, DensityAndEnergy200, ncol=2, nrow=1)
BoxGraph <- ggarrange(BoxGraph105, BoxGraph200, ncol=2, nrow=1, common.legend=TRUE)

FinalGraph <- ggarrange(Speedup, DensityAndEnergy, BoxGraph, ncol=1, nrow=3)

ggsave(FinalGraph, filename="pdfs/FinalGraph.svg", width=15, height=20)
ggsave(FinalGraph, filename="pdfs/FinalGraph.pdf",width=15, height=20)
ggsave(BoxGraph, filename="pdfs/BoxGraph.svg", width=15, height=7)
ggsave(BoxGraph, filename="pdfs/BoxGraph.pdf", width=15, height=7)
ggsave(Speedup, filename="pdfs/Speedup.svg", width=15, height=7)
ggsave(Speedup, filename="pdfs/Speedup.pdf", width=15, height=7)
