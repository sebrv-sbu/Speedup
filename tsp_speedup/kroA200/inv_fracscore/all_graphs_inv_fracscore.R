...
data_parallel_h <- bind_rows(data_20_h, data_100_h)
...
data_univ$yax    = E_min/data_univ$E

max_yax=max(data_univ$yax)
c_max_yax=floor(max_yax*K_E)/K_E
...
K_E_max_yax=floor(K_E*max(data_univ$yax))
data_univ$bin_E <- as.integer(K_E_max_yax - ceiling(K_E*data_univ$yax))
...
#  plot <- plot + scale_size_continuous(range=c(0.1, 2))
    ...
    plot <- plot + scale_x_reverse(name="log(I*P)",breaks=seq(0, max(melted_int_data$bin_I), by=4),expand=c(0,0),labels= lapply(seq(0, max(melted_int_data$bin_I), by=4), transform_bin_I))
...
temp_200_matrix <- matrix(c(1000, 100, 50, 100, 10,1000, 1000, 1000, 100, 100), ncol=2,byrow=TRUE)
...
new_data_frame_100_h <- data.frame(m.combs)
...
new_data_frame <- rbind(new_data_frame, new_data_frame_20_h, new_data_frame_100_h)
...
#new_data_frame <- rbind(new_data_frame, new_data_frame_20_h, new_data_frame_100_h, new_data_frame_100_h)
...
new_data_frame$bin_E = new_data_frame$bin_E + 1
...
hellinger_distance_cdr <- function(interval, P, lambda){
  ...
  for (bin in ((max_bin_E+1):max(as.numeric(serial_data$bin)))){
    row_s <- serial_data[which(bin == serial_data$bin),]
...
dist_data_frame_100_h <- data.frame(temp_200_matrix)
...
#dist_data_frame_200_h <- data.frame(temp_200_matrix)
...
#
dist_data_frame <- rbind(dist_data_frame, dist_data_frame_20_h, dist_data_frame_100_h)
...
plot1 <- plot1 + ggtitle("CDR kroA200")
