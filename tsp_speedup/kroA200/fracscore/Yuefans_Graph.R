...
print(tot_int_data[which(tot_int_data$P == 200),])
...
YuefanPlot <- YuefanPlot + ggtitle("Speedup Comparison 200 City Test Problem")
...
NewPlot <- NewPlot + scale_x_reverse(name="E/E_min", 
                                     ...
                                     labels=lapply(seq(max_bin_E,min_bin_E, by = -2),
                                                   transform_bin_E))
NewPlot <- NewPlot + ggtitle("200 City Final Energy Distribution")
#ggsave(NewPlot, filename="pdfs/DensityAndEnergy.pdf", height=5, width=5*1.5)
#ggsave(NewPlot, filename="pdfs/DensityAndEnergy.svg", height=5, width=5*1.5)
...
BoxGraph <- BoxGraph + ggtitle("Energy Comparison 200 City Comparison")
