...
plot <- plot + scale_color_gradientn(colors=c('orange','purple','green'),values=rescale(c(1,1.07,max(lambda_data$bin))))
...
for (proc in procs){
  ...
  m_bin         <- (max(int_data[!is.na(int_data$log_I),]$log_I)-c_bin)/2
