...
data_s$I <- data_s$I - init_moves

data_univ <- bind_rows(data_s, cdr_temp, hcdr_temp)
data_univ$E <- data_univ$E - max_energy
data_univ$log_I      <- log(data_univ$I)
data_univ$log_work   <- data_univ$log_I + log(data_univ$P)
data_univ$frac_score <- (min_energy - max_energy) / data_univ$E
data_univ$energy_bin <- ceiling(
  energy_bin_resolution * (data_univ$frac_score - 1)
)
...
problematic <- data_univ |>
  filter(1 == P) |>
  semi_join(
    work_per_run |>
      ungroup() |>
      filter(1 == P & (mean_iter <= 0 | is.na(mean_iter)))
  )
print(problematic)  # check which lambda values appear here
