...
max_energy_of_bin <- function(bin) {
  -1 * abs((((bin / energy_bin_resolution) + 1) + min_energy) * min_energy)
}
...
max_frac_score_of_bin <- function(bin) {
  abs((max_energy_of_bin(bin) - min_energy) / min_energy)
}
...
min_wasserstein_distance <- function(row_data, serial_energy) {
  if ("serial" == row_data$scheme) {
    list(distance = NA, min = NA)
  } else {
    row_densities <- as.vector(
      t(
        row_data[1, -(1:5)] |> unlist() |> na.omit()
      )
    )
    serial_energy |> rowwise() |> print()
    result <- serial_energy |>
      rowwise() |>
      mutate(
        distance = wasserstein1d(
          as.vector(
            t(
              pick(everything())[1, -(1:5)] |> unlist() |> na.omit()
            )
          ), row_densities,
        )
      ) |>
      ...
      select(distance, lambda)

    list(distance = result$distance[1],
         min = result$lambda[1])
  }
}
